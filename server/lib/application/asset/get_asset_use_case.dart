...
class GetAssetUseCase {
  ...
  const GetAssetUseCase(this._repository);

  /// Returns the [Asset] with [id] scoped to [entityId].
  ///
  /// Throws [AssetNotFoundException] if not found.
  Future<Asset> execute(String id, {required String entityId}) async {
    final asset = await _repository.findById(id, entityId: entityId);
